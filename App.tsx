...
function App() {
  ...
  // FIX: Renamed state variable to avoid conflict with window.history
      ...
      if (location.hash.startsWith('#view-report?data=')) {
        ...
        if (encodedData) {
          ...
          try {
            ...
            // FIX: Use window.history to avoid conflict with state variable.
            window.history.pushState("", document.title, window.location.pathname + window.location.search);
          } finally {
            setIsLoading(false);
          }
        }
      } else if (!location.hash.startsWith('#generate-image')) {
        setActiveReport(null);
      }
    };
  ...
  const handleBack = () => {
    setActiveReport(null);
    // Clear the hash to return to the main uploader URL
    // FIX: Use window.history to avoid conflict with state variable.
    window.history.pushState("", document.title, window.location.pathname + window.location.search);
    setActiveHash('');
  };
  ...
  const handleShare = async () => {
    ...
    try {
      const encodedData = await compressAndEncode(activeReport.markdown);
  ...
  const handleDownload = () => {
    if (!activeReport) return;
    const reportHtml = generateReportHtml(activeReport.markdown, activeReport.imageUrl);
  ...
  const handleDownloadCsv = () => {
    if (!activeReport) return;
    generateReportCsv(activeReport.markdown);
  };

  const isUploaderView = !activeReport && !activeHash.startsWith('#generate-image');
  let pageTitle;
  if(activeReport) {
    pageTitle = "AI-Powered Diagnostic Report";
  } else if (activeHash.startsWith('#generate-image')) {
    pageTitle = "AI-Powered Image Generation";
  } else {
    pageTitle = "Upload or capture a leaf image to generate an instant diagnostic report.";
  }
    ...
    <div className={`min-h-screen text-gray-800 p-4 sm:p-8 flex flex-col items-center justify-start transition-all duration-500 ease-in-out ${
      ...
      <div className="w-full max-w-5xl">
        ...
        {activeReport ? (
          ...
          />
        ) : activeHash.startsWith('#generate-image') ? (
          ...
          <>
                 ...
                 <input
                  ...
                  {!previewUrl ? (
                     <div className="w-full flex flex-col sm:flex-row gap-4">
                        ...
                        <a href="#generate-image" className="flex-1 cursor-pointer w-full border-2 border-dashed border-gray-300 rounded-lg flex flex-col items-center justify-center text-gray-500 hover:border-green-500 hover:text-green-500 transition-colors p-8 text-center no-underline">
                            <ImageIcon className="w-10 h-10 mb-2"/>
                            <span className="font-semibold">Generate an Image</span>
                            <span className="text-sm">Using AI</span>
                        </a>
                    </div>
                  ) : (
            ...
            {/* FIX: Use renamed state variable */}
